...
module.exports = {
    ...
    execute: async ({ interaction, client }) => {
        ...
        try {
            if (!MusicDispatcher.player.paused) return interaction.reply({ embeds: [client.util.embed().setDescription('Player is Already Resumed')] });
            await MusicDispatcher.resume();
            interaction.reply({ embeds: [client.util.embed().setDescription('Player Is Noww Resumed')] });
        } catch (err) {
